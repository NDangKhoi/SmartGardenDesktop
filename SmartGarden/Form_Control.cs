...
namespace SmartGarden
{
    public partial class Form_Control : Form
    {
        ...
        async void send_Limit(String path, TextBox txtLOW,TextBox txtHIGH ,String typeval,Label lb, double limitVal)
        {
            ...
            if (typeval == "LOW")
            {
                if (valLOW != "" && double.Parse(valLOW) >= 0 && double.Parse(valLOW) <= limitVal)
                {
                ...
                {
                    MessageBox.Show("Vui lòng nhập giá trị!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                if (valHIGH != "" && double.Parse(valHIGH) >= 0 && double.Parse(valHIGH) <= limitVal)
                {
